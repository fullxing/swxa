...
static int init(EVP_MD_CTX *ctx)
{
    ...
    if(rv != SDR_OK)
    {
        printf("´ò¿ª»á»°¾ä±ú´íÎó£¬´íÎóÂë[0x%08x]\n", rv);
	...
	rv = SDF_HashInit(handle->hSessionHandle, SGD_SHA1, NULL, NULL, 0);
        if(rv)
        {
            printf("SDF_HashInitº¯Êý´íÎó£¬´íÎóÂë[0x%08x]\n", rv);
...
static int update(EVP_MD_CTX *ctx,const void *data,size_t count)
{
    ...
    if(rv)
    {
    	printf("SDF_HashUpdateº¯Êý´íÎó£¬´íÎóÂë[0x%08x]\n", rv);
...
static int final(EVP_MD_CTX *ctx,unsigned char *md)
{
	...
	rv = SDF_HashFinal(handle->hSessionHandle, md, &md_length);
        if(rv)
        {
            printf("SDF_HashFinalº¯Êý´íÎó£¬´íÎóÂë[0x%08x]\n", rv);
